...
class NewsSpider(scrapy.Spider):
    ...
    start_urls = [
        'https://www.newsfirst.lk/category/business/'
    ]

    def parse(self, response):
        #from small colums
        for news in response.xpath('//div[@class="sub-1-news-block cat-bar-business-full"]'):
                print "small"   
                item= CrawlerItem()
        ...
        #from big colums
        for news in response.xpath('//div[@class="main-news-block cat-bar-business-full"]'):
                print "big"   
                item= CrawlerItem()
                item['link'] =news.css('a::attr(href)').extract_first() #link 
